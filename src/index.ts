...
export class SunsynkPowerFlowCard extends LitElement {
	...
	render() {
		...
		try {
			...
			const _stateDayBattery3Discharge = this.getEntity(
				'entities.day_battery3_discharge_71',
			);
			const _stateDayBattery3Charge = this.getEntity(
				'entities.day_battery3_charge_70',
			);
			const _stateDayBattery4Discharge = this.getEntity(
				'entities.day_battery4_discharge_71',
			);
			const _stateDayBattery4Charge = this.getEntity(
			...
			const stateInverterStatus = this.getEntity(
				'entities.inverter_status_59',
				{
					state: '',
				},
			);
			...
			//Battery 1
			const stateBatteryVoltage = this.getEntity(
			...
			const stateBatteryCurrent = this.getEntity(
			...
			const stateSOCEndOfCharge2 = this.getEntity(
				'battery2.soc_end_of_charge',
				{
			...
			const _stateBattery3CurrentDirection = this.getEntity(
			...
			const stateSOCEndOfCharge3 = this.getEntity(
				'battery3.soc_end_of_charge',
				{
			...
			//Battery 4
			const stateBattery4Voltage = this.getEntity(
				'entities.battery4_voltage_183',
			);
			const stateBattery4Soc = this.getEntity(
				'entities.battery4_soc_184',
				null,
				config.battery4.soc_decimal_places,
				Percentage.PERCENTAGE,
			);
			const stateBattery4Power = this.getEntity('entities.battery4_power_190');
			const stateBattery4Current = this.getEntity(
				'entities.battery4_current_191',
			);
			const stateBattery4Temp = this.getEntity('entities.battery4_temp_182', {
				state: '',
			});
			const stateBattery4Status = this.getEntity('entities.battery4_status', {
				state: '',
			});
			const _stateBattery4CurrentDirection = this.getEntity(
				'entities.battery4_current_direction',
				{ state: '' },
			);
			const stateBattery4RatedCapacity = this.getEntity(
				'entities.battery4_rated_capacity',
				{ state: '' },
			);
			const stateShutdownSOC4 = this.getEntity('battery4.shutdown_soc', {
				state: config.battery4.shutdown_soc?.toString() ?? '',
			});
			const stateShutdownSOCOffGrid4 = this.getEntity(
				'battery4.shutdown_soc_offgrid',
				{
					state: config.battery4.shutdown_soc_offgrid?.toString() ?? '',
				},
			);
			const stateBattery4SOH = this.getEntity('entities.battery4_soh', {
				state: '',
			});
			const stateSOCEndOfCharge4 = this.getEntity(
				'battery4.soc_end_of_charge',
				{
			...
			const battery4ColourConfig = this.colourConvert(config.battery4?.colour);
			...
			const battery4ChargeColour = this.colourConvert(
			...
			let batteryPower = stateBatteryPower.toPower(
			...
			const shutdownOffGrid4 =
				stateShutdownSOCOffGrid4.toNum() || shutdownOffGrid;
			...
			switch (true) {
				...
				default: {
					...
					function assignInverterProgramBasedOnTime(timer_now: Date) {
						...
						[prog1, prog2, prog3, prog4, prog5, prog6].forEach(
							(prog, index) => {
								...
								const nextIndex =
									(index + 1) %
									[prog1, prog2, prog3, prog4, prog5, prog6].length;
			...
			const batt4energy = this.getEntity('battery4.energy', {
			...
			if (config.show_battery || batteryEnergy !== 0 || battery2Energy !== 0) {
				...
				if (battery3Energy !== 0) {
					...
					const hours3 = Math.floor(
						(totalSeconds3 % (60 * 60 * 24)) / (60 * 60),
					);
				...
				if (battery4Energy !== 0) {
					...
					const hours4 = Math.floor(
						(totalSeconds4 % (60 * 60 * 24)) / (60 * 60),
					);
			...
			const maxLineWidth =
				(Utils.toNum(config.max_line_width) < 1 ? 1 : config.max_line_width) -
				1;
			...
			switch (true) {
				case pvPercentageBat >= Utils.toNum(config.battery?.path_threshold, 0):
					...
					break;
				case gridPercentageBat >=
					Utils.toNum(config.battery?.path_threshold, 0):
			...
			const getLoadColour = (essentialLoads, defaultColour) =>
				areLoadsBelowThreshold(...essentialLoads)
					? 'transparent'
					: defaultColour;
